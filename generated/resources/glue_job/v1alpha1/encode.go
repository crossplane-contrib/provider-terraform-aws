...
package v1alpha1func EncodeGlueJob(r GlueJob) cty.Value {
...
func EncodeGlueJob_Command(p *Command, vals map[string]cty.Value) {
	...
	for _, v := range p.Command {
		...
		EncodeGlueJob_Command_Name(v, ctyVal)
		EncodeGlueJob_Command_PythonVersion(v, ctyVal)
		EncodeGlueJob_Command_ScriptLocation(v, ctyVal)
...
func EncodeGlueJob_ExecutionProperty(p *ExecutionProperty, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ExecutionProperty {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueJob_ExecutionProperty_MaxConcurrentRuns(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueJob_NotificationProperty(p *NotificationProperty, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.NotificationProperty {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueJob_NotificationProperty_NotifyDelayAfter(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueJob_NotificationProperty_NotifyDelayAfter(p *NotificationProperty, vals map[string]cty.Value) {
	vals["notify_delay_after"] = cty.IntVal(p.NotifyDelayAfter)
}
