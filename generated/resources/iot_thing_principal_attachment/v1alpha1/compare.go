...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*IotThingPrincipalAttachment)
	p := prov.(*IotThingPrincipalAttachment)
	md := &plugin.MergeDescription{}
	updated := false
	anyChildUpdated := false

	updated = MergeIotThingPrincipalAttachment_Id(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
...
func MergeIotThingPrincipalAttachment_Id(k *IotThingPrincipalAttachmentParameters, p *IotThingPrincipalAttachmentParameters, md *plugin.MergeDescription) bool {
	if k.Id != p.Id {
		p.Id = k.Id
