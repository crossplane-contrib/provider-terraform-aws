...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*DxHostedTransitVirtualInterfaceAccepter)
	p := prov.(*DxHostedTransitVirtualInterfaceAccepter)
	...
	updated = MergeDxHostedTransitVirtualInterfaceAccepter_Timeouts(&k.Spec.ForProvider.Timeouts, &p.Spec.ForProvider.Timeouts, md)
	if updated {
		anyChildUpdated = true
	}

	updated = MergeDxHostedTransitVirtualInterfaceAccepter_Arn(&k.Status.AtProvider, &p.Status.AtProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
func MergeDxHostedTransitVirtualInterfaceAccepter_Arn(k *DxHostedTransitVirtualInterfaceAccepterObservation, p *DxHostedTransitVirtualInterfaceAccepterObservation, md *plugin.MergeDescription) bool {
	if k.Arn != p.Arn {
		k.Arn = p.Arn
