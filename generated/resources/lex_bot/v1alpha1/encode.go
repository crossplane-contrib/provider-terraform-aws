...
package v1alpha1func EncodeLexBot(r LexBot) cty.Value {
...
func EncodeLexBot_IdleSessionTtlInSeconds(p *LexBotParameters, vals map[string]cty.Value) {
	vals["idle_session_ttl_in_seconds"] = cty.IntVal(p.IdleSessionTtlInSeconds)
}
...
func EncodeLexBot_NluIntentConfidenceThreshold(p *LexBotParameters, vals map[string]cty.Value) {
	vals["nlu_intent_confidence_threshold"] = cty.IntVal(p.NluIntentConfidenceThreshold)
}
...
func EncodeLexBot_AbortStatement(p *AbortStatement, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.AbortStatement {
		ctyVal = make(map[string]cty.Value)
		EncodeLexBot_AbortStatement_ResponseCard(v, ctyVal)
		EncodeLexBot_AbortStatement_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexBot_ClarificationPrompt(p *ClarificationPrompt, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ClarificationPrompt {
		ctyVal = make(map[string]cty.Value)
		EncodeLexBot_ClarificationPrompt_MaxAttempts(v, ctyVal)
		EncodeLexBot_ClarificationPrompt_ResponseCard(v, ctyVal)
		EncodeLexBot_ClarificationPrompt_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
