...
package v1alpha1func EncodeVpcPeeringConnectionAccepter(r VpcPeeringConnectionAccepter) cty.Value {
...
func EncodeVpcPeeringConnectionAccepter_Accepter(p *Accepter, vals map[string]cty.Value) {
	...
	for _, v := range p.Accepter {
		...
		EncodeVpcPeeringConnectionAccepter_Accepter_AllowVpcToRemoteClassicLink(v, ctyVal)
		EncodeVpcPeeringConnectionAccepter_Accepter_AllowClassicLinkToRemoteVpc(v, ctyVal)
		EncodeVpcPeeringConnectionAccepter_Accepter_AllowRemoteVpcDnsResolution(v, ctyVal)
...
func EncodeVpcPeeringConnectionAccepter_Requester(p *Requester, vals map[string]cty.Value) {
	...
	for _, v := range p.Requester {
		ctyVal = make(map[string]cty.Value)
		EncodeVpcPeeringConnectionAccepter_Requester_AllowClassicLinkToRemoteVpc(v, ctyVal)
		EncodeVpcPeeringConnectionAccepter_Requester_AllowRemoteVpcDnsResolution(v, ctyVal)
		EncodeVpcPeeringConnectionAccepter_Requester_AllowVpcToRemoteClassicLink(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
