...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*CloudwatchLogSubscriptionFilter)
	p := prov.(*CloudwatchLogSubscriptionFilter)
	...
	updated = MergeCloudwatchLogSubscriptionFilter_Name(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}

	updated = MergeCloudwatchLogSubscriptionFilter_RoleArn(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
//mergePrimitiveTemplateSpec
func MergeCloudwatchLogSubscriptionFilter_Name(k *CloudwatchLogSubscriptionFilterParameters, p *CloudwatchLogSubscriptionFilterParameters, md *plugin.MergeDescription) bool {
	if k.Name != p.Name {
		p.Name = k.Name
		md.NeedsProviderUpdate = true
		return true
