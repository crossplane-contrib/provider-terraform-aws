...
package v1alpha1func EncodeDbInstance(r DbInstance) cty.Value {
...
func EncodeDbInstance_MonitoringInterval(p *DbInstanceParameters, vals map[string]cty.Value) {
	vals["monitoring_interval"] = cty.IntVal(p.MonitoringInterval)
}
...
func EncodeDbInstance_MaxAllocatedStorage(p *DbInstanceParameters, vals map[string]cty.Value) {
	vals["max_allocated_storage"] = cty.IntVal(p.MaxAllocatedStorage)
}
...
func EncodeDbInstance_PerformanceInsightsRetentionPeriod(p *DbInstanceParameters, vals map[string]cty.Value) {
	vals["performance_insights_retention_period"] = cty.IntVal(p.PerformanceInsightsRetentionPeriod)
}
...
func EncodeDbInstance_S3Import(p *S3Import, vals map[string]cty.Value) {
	...
	for _, v := range p.S3Import {
		ctyVal = make(map[string]cty.Value)
		EncodeDbInstance_S3Import_SourceEngine(v, ctyVal)
		EncodeDbInstance_S3Import_SourceEngineVersion(v, ctyVal)
		EncodeDbInstance_S3Import_BucketName(v, ctyVal)
		EncodeDbInstance_S3Import_BucketPrefix(v, ctyVal)
		EncodeDbInstance_S3Import_IngestionRole(v, ctyVal)
