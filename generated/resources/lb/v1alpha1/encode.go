...
package v1alpha1func EncodeLb(r Lb) cty.Value {
...
func EncodeLb_AccessLogs(p *AccessLogs, vals map[string]cty.Value) {
	...
	for _, v := range p.AccessLogs {
		...
		EncodeLb_AccessLogs_Prefix(v, ctyVal)
		EncodeLb_AccessLogs_Bucket(v, ctyVal)
		EncodeLb_AccessLogs_Enabled(v, ctyVal)
...
func EncodeLb_SubnetMapping(p *SubnetMapping, vals map[string]cty.Value) {
	...
	for _, v := range p.SubnetMapping {
		ctyVal = make(map[string]cty.Value)
		EncodeLb_SubnetMapping_AllocationId(v, ctyVal)
		EncodeLb_SubnetMapping_OutpostId(v, ctyVal)
		EncodeLb_SubnetMapping_PrivateIpv4Address(v, ctyVal)
		EncodeLb_SubnetMapping_SubnetId(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
