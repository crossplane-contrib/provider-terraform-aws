...
package v1alpha1func EncodeEksNodeGroup(r EksNodeGroup) cty.Value {
...
func EncodeEksNodeGroup_DiskSize(p *EksNodeGroupParameters, vals map[string]cty.Value) {
	vals["disk_size"] = cty.IntVal(p.DiskSize)
}
...
func EncodeEksNodeGroup_ScalingConfig(p *ScalingConfig, vals map[string]cty.Value) {
	...
	for _, v := range p.ScalingConfig {
		ctyVal = make(map[string]cty.Value)
		EncodeEksNodeGroup_ScalingConfig_DesiredSize(v, ctyVal)
		EncodeEksNodeGroup_ScalingConfig_MaxSize(v, ctyVal)
		EncodeEksNodeGroup_ScalingConfig_MinSize(v, ctyVal)
...
func EncodeEksNodeGroup_ScalingConfig_DesiredSize(p *ScalingConfig, vals map[string]cty.Value) {
	vals["desired_size"] = cty.IntVal(p.DesiredSize)
}
...
func EncodeEksNodeGroup_LaunchTemplate(p *LaunchTemplate, vals map[string]cty.Value) {
	...
	for _, v := range p.LaunchTemplate {
		ctyVal = make(map[string]cty.Value)
		EncodeEksNodeGroup_LaunchTemplate_Id(v, ctyVal)
		EncodeEksNodeGroup_LaunchTemplate_Name(v, ctyVal)
		EncodeEksNodeGroup_LaunchTemplate_Version(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeEksNodeGroup_RemoteAccess(p *RemoteAccess, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.RemoteAccess {
		ctyVal = make(map[string]cty.Value)
		EncodeEksNodeGroup_RemoteAccess_Ec2SshKey(v, ctyVal)
		EncodeEksNodeGroup_RemoteAccess_SourceSecurityGroupIds(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeEksNodeGroup_Resources_AutoscalingGroups(p *AutoscalingGroups, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.AutoscalingGroups {
		ctyVal = make(map[string]cty.Value)
