...
func (e *ctyEncoder) EncodeCty(mr resource.Managed, schema *providers.Schema) (cty.Value, error) {
	r, ok := mr.(*AcmCertificate)
	if !ok {
		return cty.NilVal, fmt.Errorf("EncodeType received a resource.Managed value which is not a AcmCertificate.")
	}
	return EncodeAcmCertificate(*r), nil
}
...
func EncodeAcmCertificate_Options(p Options, vals map[string]cty.Value) {
	...
	ctyVal := make(map[string]cty.Value)
	EncodeAcmCertificate_Options_CertificateTransparencyLoggingPreference(p, ctyVal)
