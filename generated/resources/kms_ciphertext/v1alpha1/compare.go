...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	...
	updated = MergeKmsCiphertext_Id(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
func MergeKmsCiphertext_Context(k *KmsCiphertextParameters, p *KmsCiphertextParameters, md *plugin.MergeDescription) bool {
	if !plugin.CompareMapString(p.Context, p.Context) {
		p.Context = k.Context
		md.NeedsProviderUpdate = true
		return true
	}
...
//mergePrimitiveTemplateSpec
func MergeKmsCiphertext_Id(k *KmsCiphertextParameters, p *KmsCiphertextParameters, md *plugin.MergeDescription) bool {
	if k.Id != p.Id {
		p.Id = k.Id
		md.NeedsProviderUpdate = true
		return true
