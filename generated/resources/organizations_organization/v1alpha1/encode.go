...
package v1alpha1func EncodeOrganizationsOrganization(r OrganizationsOrganization) cty.Value {
...
func EncodeOrganizationsOrganization_NonMasterAccounts(p *NonMasterAccounts, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.NonMasterAccounts {
		ctyVal = make(map[string]cty.Value)
