...
package v1alpha1func EncodeSqsQueue(r SqsQueue) cty.Value {
...
func EncodeSqsQueue_VisibilityTimeoutSeconds(p *SqsQueueParameters, vals map[string]cty.Value) {
	vals["visibility_timeout_seconds"] = cty.IntVal(p.VisibilityTimeoutSeconds)
}
...
func EncodeSqsQueue_KmsDataKeyReusePeriodSeconds(p *SqsQueueParameters, vals map[string]cty.Value) {
	vals["kms_data_key_reuse_period_seconds"] = cty.IntVal(p.KmsDataKeyReusePeriodSeconds)
}
