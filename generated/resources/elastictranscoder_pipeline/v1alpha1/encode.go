...
func (e *ctyEncoder) EncodeCty(mr resource.Managed, schema *providers.Schema) (cty.Value, error) {
	r, ok := mr.(*ElastictranscoderPipeline)
	if !ok {
		return cty.NilVal, fmt.Errorf("EncodeType received a resource.Managed value which is not a ElastictranscoderPipeline.")
	}
	return EncodeElastictranscoderPipeline(*r), nil
}
...
func EncodeElastictranscoderPipeline_ContentConfigPermissions(p ContentConfigPermissions, vals map[string]cty.Value) {
	valsForCollection := make([]cty.Value, 1)
	ctyVal := make(map[string]cty.Value)
	EncodeElastictranscoderPipeline_ContentConfigPermissions_Access(p, ctyVal)
	EncodeElastictranscoderPipeline_ContentConfigPermissions_Grantee(p, ctyVal)
	EncodeElastictranscoderPipeline_ContentConfigPermissions_GranteeType(p, ctyVal)
	valsForCollection[0] = cty.ObjectVal(ctyVal)
...
func EncodeElastictranscoderPipeline_ThumbnailConfig(p ThumbnailConfig, vals map[string]cty.Value) {
	valsForCollection := make([]cty.Value, 1)
	ctyVal := make(map[string]cty.Value)
	EncodeElastictranscoderPipeline_ThumbnailConfig_Bucket(p, ctyVal)
	EncodeElastictranscoderPipeline_ThumbnailConfig_StorageClass(p, ctyVal)
	valsForCollection[0] = cty.ObjectVal(ctyVal)
