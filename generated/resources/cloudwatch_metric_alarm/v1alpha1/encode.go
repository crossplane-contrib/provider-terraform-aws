...
func (e *ctyEncoder) EncodeCty(mr resource.Managed, schema *providers.Schema) (cty.Value, error) {
	r, ok := mr.(*CloudwatchMetricAlarm)
	if !ok {
		return cty.NilVal, fmt.Errorf("EncodeType received a resource.Managed value which is not a CloudwatchMetricAlarm.")
	}
	return EncodeCloudwatchMetricAlarm(*r), nil
}
...
func EncodeCloudwatchMetricAlarm_Dimensions(p CloudwatchMetricAlarmParameters, vals map[string]cty.Value) {
	if len(p.Dimensions) == 0 {
		vals["dimensions"] = cty.NullVal(cty.Map(cty.String))
		return
	}
