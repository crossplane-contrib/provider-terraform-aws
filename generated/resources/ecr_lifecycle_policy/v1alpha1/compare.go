...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*EcrLifecyclePolicy)
	p := prov.(*EcrLifecyclePolicy)
	...
	updated = MergeEcrLifecyclePolicy_Id(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}

	updated = MergeEcrLifecyclePolicy_RegistryId(&k.Status.AtProvider, &p.Status.AtProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
//mergePrimitiveTemplateSpec
func MergeEcrLifecyclePolicy_Id(k *EcrLifecyclePolicyParameters, p *EcrLifecyclePolicyParameters, md *plugin.MergeDescription) bool {
	if k.Id != p.Id {
		p.Id = k.Id
		md.NeedsProviderUpdate = true
		return true
...
func MergeEcrLifecyclePolicy_RegistryId(k *EcrLifecyclePolicyObservation, p *EcrLifecyclePolicyObservation, md *plugin.MergeDescription) bool {
