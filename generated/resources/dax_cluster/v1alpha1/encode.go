...
package v1alpha1func EncodeDaxCluster(r DaxCluster) cty.Value {
...
func EncodeDaxCluster_ReplicationFactor(p *DaxClusterParameters, vals map[string]cty.Value) {
	vals["replication_factor"] = cty.IntVal(p.ReplicationFactor)
}
...
func EncodeDaxCluster_ServerSideEncryption(p *ServerSideEncryption, vals map[string]cty.Value) {
	...
	for _, v := range p.ServerSideEncryption {
		ctyVal = make(map[string]cty.Value)
		EncodeDaxCluster_ServerSideEncryption_Enabled(v, ctyVal)
