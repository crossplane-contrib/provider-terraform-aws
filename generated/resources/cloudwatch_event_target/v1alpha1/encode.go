...
func (e *ctyEncoder) EncodeCty(mr resource.Managed, schema *providers.Schema) (cty.Value, error) {
	r, ok := mr.(*CloudwatchEventTarget)
	if !ok {
		return cty.NilVal, fmt.Errorf("EncodeType received a resource.Managed value which is not a CloudwatchEventTarget.")
	}
	return EncodeCloudwatchEventTarget(*r), nil
}
...
func EncodeCloudwatchEventTarget_SqsTarget(p SqsTarget, vals map[string]cty.Value) {
	...
	ctyVal := make(map[string]cty.Value)
	EncodeCloudwatchEventTarget_SqsTarget_MessageGroupId(p, ctyVal)
...
func EncodeCloudwatchEventTarget_InputTransformer_InputPaths(p InputTransformer, vals map[string]cty.Value) {
	if len(p.InputPaths) == 0 {
		vals["input_paths"] = cty.NullVal(cty.Map(cty.String))
		return
	}
...
func EncodeCloudwatchEventTarget_KinesisTarget(p KinesisTarget, vals map[string]cty.Value) {
	valsForCollection := make([]cty.Value, 1)
	ctyVal := make(map[string]cty.Value)
	EncodeCloudwatchEventTarget_KinesisTarget_PartitionKeyPath(p, ctyVal)
	valsForCollection[0] = cty.ObjectVal(ctyVal)
