...
package v1alpha1func EncodeGlueMlTransform(r GlueMlTransform) cty.Value {
...
func EncodeGlueMlTransform_NumberOfWorkers(p *GlueMlTransformParameters, vals map[string]cty.Value) {
	vals["number_of_workers"] = cty.IntVal(p.NumberOfWorkers)
}
...
func EncodeGlueMlTransform_InputRecordTables(p *InputRecordTables, vals map[string]cty.Value) {
	...
	for _, v := range p.InputRecordTables {
...
func EncodeGlueMlTransform_Parameters(p *Parameters, vals map[string]cty.Value) {
	...
	for _, v := range p.Parameters {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueMlTransform_Parameters_TransformType(v, ctyVal)
		EncodeGlueMlTransform_Parameters_FindMatchesParameters(v.FindMatchesParameters, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueMlTransform_Parameters_FindMatchesParameters(p *FindMatchesParameters, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.FindMatchesParameters {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueMlTransform_Parameters_FindMatchesParameters_PrecisionRecallTradeOff(v, ctyVal)
		EncodeGlueMlTransform_Parameters_FindMatchesParameters_PrimaryKeyColumnName(v, ctyVal)
		EncodeGlueMlTransform_Parameters_FindMatchesParameters_AccuracyCostTradeOff(v, ctyVal)
		EncodeGlueMlTransform_Parameters_FindMatchesParameters_EnforceProvidedLabels(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueMlTransform_Parameters_FindMatchesParameters_PrecisionRecallTradeOff(p *FindMatchesParameters, vals map[string]cty.Value) {
	vals["precision_recall_trade_off"] = cty.IntVal(p.PrecisionRecallTradeOff)
}
...
func EncodeGlueMlTransform_Parameters_FindMatchesParameters_AccuracyCostTradeOff(p *FindMatchesParameters, vals map[string]cty.Value) {
	vals["accuracy_cost_trade_off"] = cty.IntVal(p.AccuracyCostTradeOff)
}
...
func EncodeGlueMlTransform_LabelCount(p *GlueMlTransformObservation, vals map[string]cty.Value) {
	vals["label_count"] = cty.IntVal(p.LabelCount)
}
