...
package v1alpha1func EncodeGlueCatalogTable(r GlueCatalogTable) cty.Value {
...
func EncodeGlueCatalogTable_Retention(p *GlueCatalogTableParameters, vals map[string]cty.Value) {
	vals["retention"] = cty.IntVal(p.Retention)
}
...
func EncodeGlueCatalogTable_PartitionKeys(p *PartitionKeys, vals map[string]cty.Value) {
	...
	for _, v := range p.PartitionKeys {
		...
		EncodeGlueCatalogTable_PartitionKeys_Name(v, ctyVal)
		EncodeGlueCatalogTable_PartitionKeys_Type(v, ctyVal)
		EncodeGlueCatalogTable_PartitionKeys_Comment(v, ctyVal)
...
func EncodeGlueCatalogTable_StorageDescriptor(p *StorageDescriptor, vals map[string]cty.Value) {
	...
	for _, v := range p.StorageDescriptor {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueCatalogTable_StorageDescriptor_BucketColumns(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Compressed(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_InputFormat(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Location(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_NumberOfBuckets(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_OutputFormat(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Parameters(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_StoredAsSubDirectories(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Columns(v.Columns, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SerDeInfo(v.SerDeInfo, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SkewedInfo(v.SkewedInfo, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SortColumns(v.SortColumns, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueCatalogTable_StorageDescriptor_Columns(p *Columns, vals map[string]cty.Value) {
	...
	for _, v := range p.Columns {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueCatalogTable_StorageDescriptor_Columns_Comment(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Columns_Name(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_Columns_Type(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueCatalogTable_StorageDescriptor_SerDeInfo(p *SerDeInfo, vals map[string]cty.Value) {
	...
	for _, v := range p.SerDeInfo {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueCatalogTable_StorageDescriptor_SerDeInfo_Name(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SerDeInfo_Parameters(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SerDeInfo_SerializationLibrary(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueCatalogTable_StorageDescriptor_SkewedInfo(p *SkewedInfo, vals map[string]cty.Value) {
	...
	for _, v := range p.SkewedInfo {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueCatalogTable_StorageDescriptor_SkewedInfo_SkewedColumnNames(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SkewedInfo_SkewedColumnValueLocationMaps(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SkewedInfo_SkewedColumnValues(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeGlueCatalogTable_StorageDescriptor_SortColumns(p *SortColumns, vals map[string]cty.Value) {
	...
	for _, v := range p.SortColumns {
		ctyVal = make(map[string]cty.Value)
		EncodeGlueCatalogTable_StorageDescriptor_SortColumns_Column(v, ctyVal)
		EncodeGlueCatalogTable_StorageDescriptor_SortColumns_SortOrder(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
