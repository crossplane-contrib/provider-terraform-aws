...
func EncodeOpsworksInstance_EphemeralBlockDevice(p EphemeralBlockDevice, vals map[string]cty.Value) {
	valsForCollection := make([]cty.Value, 1)
	ctyVal := make(map[string]cty.Value)
	EncodeOpsworksInstance_EphemeralBlockDevice_DeviceName(p, ctyVal)
	EncodeOpsworksInstance_EphemeralBlockDevice_VirtualName(p, ctyVal)
	valsForCollection[0] = cty.ObjectVal(ctyVal)
...
func EncodeOpsworksInstance_RootBlockDevice(p RootBlockDevice, vals map[string]cty.Value) {
	valsForCollection := make([]cty.Value, 1)
	ctyVal := make(map[string]cty.Value)
	EncodeOpsworksInstance_RootBlockDevice_DeleteOnTermination(p, ctyVal)
	EncodeOpsworksInstance_RootBlockDevice_Iops(p, ctyVal)
	EncodeOpsworksInstance_RootBlockDevice_VolumeSize(p, ctyVal)
	EncodeOpsworksInstance_RootBlockDevice_VolumeType(p, ctyVal)
	valsForCollection[0] = cty.ObjectVal(ctyVal)
