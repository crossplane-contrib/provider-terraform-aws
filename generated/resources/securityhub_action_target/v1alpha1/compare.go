...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*SecurityhubActionTarget)
	p := prov.(*SecurityhubActionTarget)
	...
	updated = MergeSecurityhubActionTarget_Id(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
	...
	updated = MergeSecurityhubActionTarget_Arn(&k.Status.AtProvider, &p.Status.AtProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
//mergePrimitiveTemplateSpec
func MergeSecurityhubActionTarget_Id(k *SecurityhubActionTargetParameters, p *SecurityhubActionTargetParameters, md *plugin.MergeDescription) bool {
	if k.Id != p.Id {
		p.Id = k.Id
		md.NeedsProviderUpdate = true
		return true
