...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*GuarddutyOrganizationAdminAccount)
	p := prov.(*GuarddutyOrganizationAdminAccount)
	md := &plugin.MergeDescription{}
	updated := false
	anyChildUpdated := false

	updated = MergeGuarddutyOrganizationAdminAccount_AdminAccountId(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
