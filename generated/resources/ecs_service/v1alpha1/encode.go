...
package v1alpha1func EncodeEcsService(r EcsService) cty.Value {
...
func EncodeEcsService_DeploymentMinimumHealthyPercent(p *EcsServiceParameters, vals map[string]cty.Value) {
	vals["deployment_minimum_healthy_percent"] = cty.IntVal(p.DeploymentMinimumHealthyPercent)
}
...
func EncodeEcsService_DeploymentMaximumPercent(p *EcsServiceParameters, vals map[string]cty.Value) {
	vals["deployment_maximum_percent"] = cty.IntVal(p.DeploymentMaximumPercent)
}
...
func EncodeEcsService_DesiredCount(p *EcsServiceParameters, vals map[string]cty.Value) {
	vals["desired_count"] = cty.IntVal(p.DesiredCount)
}

func EncodeEcsService_HealthCheckGracePeriodSeconds(p *EcsServiceParameters, vals map[string]cty.Value) {
	vals["health_check_grace_period_seconds"] = cty.IntVal(p.HealthCheckGracePeriodSeconds)
}

func EncodeEcsService_CapacityProviderStrategy(p *CapacityProviderStrategy, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.CapacityProviderStrategy {
		ctyVal = make(map[string]cty.Value)
		EncodeEcsService_CapacityProviderStrategy_Base(v, ctyVal)
		EncodeEcsService_CapacityProviderStrategy_CapacityProvider(v, ctyVal)
		EncodeEcsService_CapacityProviderStrategy_Weight(v, ctyVal)
...
func EncodeEcsService_DeploymentController(p *DeploymentController, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.DeploymentController {
		ctyVal = make(map[string]cty.Value)
		EncodeEcsService_DeploymentController_Type(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeEcsService_LoadBalancer(p *LoadBalancer, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.LoadBalancer {
		ctyVal = make(map[string]cty.Value)
		EncodeEcsService_LoadBalancer_ContainerName(v, ctyVal)
		EncodeEcsService_LoadBalancer_ContainerPort(v, ctyVal)
		EncodeEcsService_LoadBalancer_ElbName(v, ctyVal)
		EncodeEcsService_LoadBalancer_TargetGroupArn(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeEcsService_LoadBalancer_ContainerPort(p *LoadBalancer, vals map[string]cty.Value) {
	vals["container_port"] = cty.IntVal(p.ContainerPort)
}
...
func EncodeEcsService_NetworkConfiguration(p *NetworkConfiguration, vals map[string]cty.Value) {
	...
	for _, v := range p.NetworkConfiguration {
		ctyVal = make(map[string]cty.Value)
		EncodeEcsService_NetworkConfiguration_SecurityGroups(v, ctyVal)
		EncodeEcsService_NetworkConfiguration_Subnets(v, ctyVal)
		EncodeEcsService_NetworkConfiguration_AssignPublicIp(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeEcsService_OrderedPlacementStrategy(p *OrderedPlacementStrategy, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.OrderedPlacementStrategy {
		ctyVal = make(map[string]cty.Value)
...
func EncodeEcsService_ServiceRegistries(p *ServiceRegistries, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ServiceRegistries {
		ctyVal = make(map[string]cty.Value)
		EncodeEcsService_ServiceRegistries_RegistryArn(v, ctyVal)
		EncodeEcsService_ServiceRegistries_ContainerName(v, ctyVal)
		EncodeEcsService_ServiceRegistries_ContainerPort(v, ctyVal)
		EncodeEcsService_ServiceRegistries_Port(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
