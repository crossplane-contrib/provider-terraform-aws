...
package v1alpha1func EncodeDbOptionGroup(r DbOptionGroup) cty.Value {
...
func EncodeDbOptionGroup_Option(p *Option, vals map[string]cty.Value) {
	...
	for _, v := range p.Option {
		ctyVal = make(map[string]cty.Value)
		EncodeDbOptionGroup_Option_DbSecurityGroupMemberships(v, ctyVal)
		EncodeDbOptionGroup_Option_OptionName(v, ctyVal)
		EncodeDbOptionGroup_Option_Port(v, ctyVal)
		EncodeDbOptionGroup_Option_Version(v, ctyVal)
		EncodeDbOptionGroup_Option_VpcSecurityGroupMemberships(v, ctyVal)
		EncodeDbOptionGroup_Option_OptionSettings(v.OptionSettings, ctyVal)
...
func EncodeDbOptionGroup_Option_OptionSettings(p *OptionSettings, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.OptionSettings {
		ctyVal = make(map[string]cty.Value)
		EncodeDbOptionGroup_Option_OptionSettings_Name(v, ctyVal)
		EncodeDbOptionGroup_Option_OptionSettings_Value(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
