...
func (e *ctyEncoder) EncodeCty(mr resource.Managed, schema *providers.Schema) (cty.Value, error) {
	r, ok := mr.(*BudgetsBudget)
	if !ok {
		return cty.NilVal, fmt.Errorf("EncodeType received a resource.Managed value which is not a BudgetsBudget.")
	}
	return EncodeBudgetsBudget(*r), nil
}
...
func EncodeBudgetsBudget_CostFilters(p BudgetsBudgetParameters, vals map[string]cty.Value) {
	if len(p.CostFilters) == 0 {
		vals["cost_filters"] = cty.NullVal(cty.Map(cty.String))
		return
	}
