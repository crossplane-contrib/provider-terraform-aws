...
package v1alpha1func EncodeLexIntent(r LexIntent) cty.Value {
...
func EncodeLexIntent_Slot_ValueElicitationPrompt(p *ValueElicitationPrompt, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ValueElicitationPrompt {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_Slot_ValueElicitationPrompt_MaxAttempts(v, ctyVal)
		EncodeLexIntent_Slot_ValueElicitationPrompt_ResponseCard(v, ctyVal)
		EncodeLexIntent_Slot_ValueElicitationPrompt_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_Slot_ValueElicitationPrompt_MaxAttempts(p *ValueElicitationPrompt, vals map[string]cty.Value) {
	vals["max_attempts"] = cty.IntVal(p.MaxAttempts)
}
...
func EncodeLexIntent_Slot_ValueElicitationPrompt_Message_GroupNumber(p *Message, vals map[string]cty.Value) {
	vals["group_number"] = cty.IntVal(p.GroupNumber)
}
...
func EncodeLexIntent_ConclusionStatement(p *ConclusionStatement, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ConclusionStatement {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_ConclusionStatement_ResponseCard(v, ctyVal)
		EncodeLexIntent_ConclusionStatement_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_ConfirmationPrompt(p *ConfirmationPrompt, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.ConfirmationPrompt {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_ConfirmationPrompt_ResponseCard(v, ctyVal)
		EncodeLexIntent_ConfirmationPrompt_MaxAttempts(v, ctyVal)
		EncodeLexIntent_ConfirmationPrompt_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_DialogCodeHook(p *DialogCodeHook, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.DialogCodeHook {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_DialogCodeHook_MessageVersion(v, ctyVal)
		EncodeLexIntent_DialogCodeHook_Uri(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_FollowUpPrompt(p *FollowUpPrompt, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.FollowUpPrompt {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_FollowUpPrompt_Prompt(v.Prompt, ctyVal)
		EncodeLexIntent_FollowUpPrompt_RejectionStatement(v.RejectionStatement, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_FollowUpPrompt_Prompt(p *Prompt, vals map[string]cty.Value) {
	...
	for _, v := range p.Prompt {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_FollowUpPrompt_Prompt_MaxAttempts(v, ctyVal)
		EncodeLexIntent_FollowUpPrompt_Prompt_ResponseCard(v, ctyVal)
		EncodeLexIntent_FollowUpPrompt_Prompt_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_FollowUpPrompt_RejectionStatement(p *RejectionStatement, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.RejectionStatement {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_FollowUpPrompt_RejectionStatement_ResponseCard(v, ctyVal)
		EncodeLexIntent_FollowUpPrompt_RejectionStatement_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_FulfillmentActivity(p *FulfillmentActivity, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.FulfillmentActivity {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_FulfillmentActivity_Type(v, ctyVal)
		EncodeLexIntent_FulfillmentActivity_CodeHook(v.CodeHook, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_FulfillmentActivity_CodeHook(p *CodeHook, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.CodeHook {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_FulfillmentActivity_CodeHook_MessageVersion(v, ctyVal)
		EncodeLexIntent_FulfillmentActivity_CodeHook_Uri(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeLexIntent_RejectionStatement(p *RejectionStatement, vals map[string]cty.Value) {
	...
	for _, v := range p.RejectionStatement {
		ctyVal = make(map[string]cty.Value)
		EncodeLexIntent_RejectionStatement_ResponseCard(v, ctyVal)
		EncodeLexIntent_RejectionStatement_Message(v.Message, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
	vals["rejection_statement"] = cty.ListVal(valsForCollection)
}
