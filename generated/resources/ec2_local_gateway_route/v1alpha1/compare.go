...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*Ec2LocalGatewayRoute)
	p := prov.(*Ec2LocalGatewayRoute)
	md := &plugin.MergeDescription{}
	updated := false
	anyChildUpdated := false

	updated = MergeEc2LocalGatewayRoute_DestinationCidrBlock(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
	...
	updated = MergeEc2LocalGatewayRoute_LocalGatewayVirtualInterfaceGroupId(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
func MergeEc2LocalGatewayRoute_LocalGatewayVirtualInterfaceGroupId(k *Ec2LocalGatewayRouteParameters, p *Ec2LocalGatewayRouteParameters, md *plugin.MergeDescription) bool {
	if k.LocalGatewayVirtualInterfaceGroupId != p.LocalGatewayVirtualInterfaceGroupId {
		p.LocalGatewayVirtualInterfaceGroupId = k.LocalGatewayVirtualInterfaceGroupId
		md.NeedsProviderUpdate = true
		return true
	}
	return false
