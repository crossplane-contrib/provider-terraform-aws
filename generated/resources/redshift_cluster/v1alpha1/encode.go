...
package v1alpha1func EncodeRedshiftCluster(r RedshiftCluster) cty.Value {
...
func EncodeRedshiftCluster_NumberOfNodes(p *RedshiftClusterParameters, vals map[string]cty.Value) {
	vals["number_of_nodes"] = cty.IntVal(p.NumberOfNodes)
}
...
func EncodeRedshiftCluster_AutomatedSnapshotRetentionPeriod(p *RedshiftClusterParameters, vals map[string]cty.Value) {
	vals["automated_snapshot_retention_period"] = cty.IntVal(p.AutomatedSnapshotRetentionPeriod)
}
...
func EncodeRedshiftCluster_Logging(p *Logging, vals map[string]cty.Value) {
	...
	for _, v := range p.Logging {
		...
		EncodeRedshiftCluster_Logging_BucketName(v, ctyVal)
		EncodeRedshiftCluster_Logging_Enable(v, ctyVal)
		EncodeRedshiftCluster_Logging_S3KeyPrefix(v, ctyVal)
...
func EncodeRedshiftCluster_SnapshotCopy(p *SnapshotCopy, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.SnapshotCopy {
		ctyVal = make(map[string]cty.Value)
		EncodeRedshiftCluster_SnapshotCopy_DestinationRegion(v, ctyVal)
		EncodeRedshiftCluster_SnapshotCopy_GrantName(v, ctyVal)
		EncodeRedshiftCluster_SnapshotCopy_RetentionPeriod(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
