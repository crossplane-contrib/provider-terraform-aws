...
package v1alpha1func EncodeCognitoUserPoolClient(r CognitoUserPoolClient) cty.Value {
...
func EncodeCognitoUserPoolClient_RefreshTokenValidity(p *CognitoUserPoolClientParameters, vals map[string]cty.Value) {
	vals["refresh_token_validity"] = cty.IntVal(p.RefreshTokenValidity)
}

func EncodeCognitoUserPoolClient_AnalyticsConfiguration(p *AnalyticsConfiguration, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.AnalyticsConfiguration {
		ctyVal = make(map[string]cty.Value)
		EncodeCognitoUserPoolClient_AnalyticsConfiguration_ExternalId(v, ctyVal)
		EncodeCognitoUserPoolClient_AnalyticsConfiguration_RoleArn(v, ctyVal)
		EncodeCognitoUserPoolClient_AnalyticsConfiguration_UserDataShared(v, ctyVal)
		EncodeCognitoUserPoolClient_AnalyticsConfiguration_ApplicationId(v, ctyVal)
