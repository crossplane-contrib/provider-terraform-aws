...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	...
	updated = MergeApigatewayv2Route_Id(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
func MergeApigatewayv2Route_RequestModels(k *Apigatewayv2RouteParameters, p *Apigatewayv2RouteParameters, md *plugin.MergeDescription) bool {
	if !plugin.CompareMapString(p.RequestModels, p.RequestModels) {
...
//mergePrimitiveTemplateSpec
func MergeApigatewayv2Route_Id(k *Apigatewayv2RouteParameters, p *Apigatewayv2RouteParameters, md *plugin.MergeDescription) bool {
	if k.Id != p.Id {
		p.Id = k.Id
		md.NeedsProviderUpdate = true
		return true
...
func MergeApigatewayv2Route_AuthorizationScopes(k *Apigatewayv2RouteParameters, p *Apigatewayv2RouteParameters, md *plugin.MergeDescription) bool {
	if !plugin.CompareStringSlices(p.AuthorizationScopes, p.AuthorizationScopes) {
