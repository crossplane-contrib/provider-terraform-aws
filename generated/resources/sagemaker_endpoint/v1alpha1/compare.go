...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*SagemakerEndpoint)
	p := prov.(*SagemakerEndpoint)
	...
	updated = MergeSagemakerEndpoint_Arn(&k.Status.AtProvider, &p.Status.AtProvider, md)
	if updated {
		anyChildUpdated = true
	}
...
func MergeSagemakerEndpoint_Arn(k *SagemakerEndpointObservation, p *SagemakerEndpointObservation, md *plugin.MergeDescription) bool {
	if k.Arn != p.Arn {
		k.Arn = p.Arn
