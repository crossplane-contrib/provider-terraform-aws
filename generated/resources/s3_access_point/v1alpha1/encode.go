...
package v1alpha1func EncodeS3AccessPoint(r S3AccessPoint) cty.Value {
...
func EncodeS3AccessPoint_PublicAccessBlockConfiguration(p *PublicAccessBlockConfiguration, vals map[string]cty.Value) {
	...
	for _, v := range p.PublicAccessBlockConfiguration {
		ctyVal = make(map[string]cty.Value)
		EncodeS3AccessPoint_PublicAccessBlockConfiguration_RestrictPublicBuckets(v, ctyVal)
		EncodeS3AccessPoint_PublicAccessBlockConfiguration_BlockPublicAcls(v, ctyVal)
		EncodeS3AccessPoint_PublicAccessBlockConfiguration_BlockPublicPolicy(v, ctyVal)
		EncodeS3AccessPoint_PublicAccessBlockConfiguration_IgnorePublicAcls(v, ctyVal)
...
func EncodeS3AccessPoint_VpcConfiguration(p *VpcConfiguration, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.VpcConfiguration {
		ctyVal = make(map[string]cty.Value)
		EncodeS3AccessPoint_VpcConfiguration_VpcId(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
