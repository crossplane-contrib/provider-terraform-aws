...
package v1alpha1func EncodeCodepipeline(r Codepipeline) cty.Value {
...
func EncodeCodepipeline_ArtifactStore_EncryptionKey(p *EncryptionKey, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.EncryptionKey {
		ctyVal = make(map[string]cty.Value)
		EncodeCodepipeline_ArtifactStore_EncryptionKey_Id(v, ctyVal)
		EncodeCodepipeline_ArtifactStore_EncryptionKey_Type(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeCodepipeline_Stage_Action_RunOrder(p *Action, vals map[string]cty.Value) {
	vals["run_order"] = cty.IntVal(p.RunOrder)
}
