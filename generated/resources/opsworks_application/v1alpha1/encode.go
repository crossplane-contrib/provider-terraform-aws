...
package v1alpha1func EncodeOpsworksApplication(r OpsworksApplication) cty.Value {
...
func EncodeOpsworksApplication_Environment(p *Environment, vals map[string]cty.Value) {
	...
	for _, v := range p.Environment {
		...
		EncodeOpsworksApplication_Environment_Key(v, ctyVal)
		EncodeOpsworksApplication_Environment_Secure(v, ctyVal)
		EncodeOpsworksApplication_Environment_Value(v, ctyVal)
...
func EncodeOpsworksApplication_SslConfiguration(p *SslConfiguration, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.SslConfiguration {
		ctyVal = make(map[string]cty.Value)
		EncodeOpsworksApplication_SslConfiguration_Certificate(v, ctyVal)
		EncodeOpsworksApplication_SslConfiguration_Chain(v, ctyVal)
		EncodeOpsworksApplication_SslConfiguration_PrivateKey(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeOpsworksApplication_AppSource(p *AppSource, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.AppSource {
		ctyVal = make(map[string]cty.Value)
		EncodeOpsworksApplication_AppSource_SshKey(v, ctyVal)
		EncodeOpsworksApplication_AppSource_Type(v, ctyVal)
		EncodeOpsworksApplication_AppSource_Url(v, ctyVal)
		EncodeOpsworksApplication_AppSource_Username(v, ctyVal)
		EncodeOpsworksApplication_AppSource_Password(v, ctyVal)
		EncodeOpsworksApplication_AppSource_Revision(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
