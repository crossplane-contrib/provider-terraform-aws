...
package v1alpha1func EncodeMqBroker(r MqBroker) cty.Value {
...
func EncodeMqBroker_Configuration(p *Configuration, vals map[string]cty.Value) {
	...
	for _, v := range p.Configuration {
		ctyVal = make(map[string]cty.Value)
		EncodeMqBroker_Configuration_Id(v, ctyVal)
		EncodeMqBroker_Configuration_Revision(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeMqBroker_EncryptionOptions(p *EncryptionOptions, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.EncryptionOptions {
		ctyVal = make(map[string]cty.Value)
		EncodeMqBroker_EncryptionOptions_KmsKeyId(v, ctyVal)
		EncodeMqBroker_EncryptionOptions_UseAwsOwnedKey(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeMqBroker_Logs(p *Logs, vals map[string]cty.Value) {
	...
	for _, v := range p.Logs {
		ctyVal = make(map[string]cty.Value)
		EncodeMqBroker_Logs_Audit(v, ctyVal)
		EncodeMqBroker_Logs_General(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
...
func EncodeMqBroker_MaintenanceWindowStartTime(p *MaintenanceWindowStartTime, vals map[string]cty.Value) {
	valsForCollection = make([]cty.Value, 0)
	for _, v := range p.MaintenanceWindowStartTime {
		ctyVal = make(map[string]cty.Value)
		EncodeMqBroker_MaintenanceWindowStartTime_DayOfWeek(v, ctyVal)
		EncodeMqBroker_MaintenanceWindowStartTime_TimeOfDay(v, ctyVal)
		EncodeMqBroker_MaintenanceWindowStartTime_TimeZone(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
