...
func (r *resourceMerger) MergeResources(kube resource.Managed, prov resource.Managed) plugin.MergeDescription {
	k := kube.(*StoragegatewayCache)
	p := prov.(*StoragegatewayCache)
	md := &plugin.MergeDescription{}
	updated := false
	anyChildUpdated := false

	updated = MergeStoragegatewayCache_DiskId(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}

	updated = MergeStoragegatewayCache_GatewayArn(&k.Spec.ForProvider, &p.Spec.ForProvider, md)
	if updated {
		anyChildUpdated = true
	}
