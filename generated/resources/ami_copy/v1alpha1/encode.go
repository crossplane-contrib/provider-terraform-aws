...
package v1alpha1func EncodeAmiCopy(r AmiCopy) cty.Value {
...
func EncodeAmiCopy_EbsBlockDevice(p *EbsBlockDevice, vals map[string]cty.Value) {
	...
	for _, v := range p.EbsBlockDevice {
		...
		EncodeAmiCopy_EbsBlockDevice_DeleteOnTermination(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_DeviceName(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_Encrypted(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_Iops(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_SnapshotId(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_VolumeSize(v, ctyVal)
		EncodeAmiCopy_EbsBlockDevice_VolumeType(v, ctyVal)
...
func EncodeAmiCopy_EphemeralBlockDevice(p *EphemeralBlockDevice, vals map[string]cty.Value) {
	...
	for _, v := range p.EphemeralBlockDevice {
		ctyVal = make(map[string]cty.Value)
		EncodeAmiCopy_EphemeralBlockDevice_DeviceName(v, ctyVal)
		EncodeAmiCopy_EphemeralBlockDevice_VirtualName(v, ctyVal)
		valsForCollection = append(valsForCollection, cty.ObjectVal(ctyVal))
	}
