...
package v1alpha1func EncodeWorkspacesWorkspace(r WorkspacesWorkspace) cty.Value {
...
func EncodeWorkspacesWorkspace_WorkspaceProperties(p *WorkspaceProperties, vals map[string]cty.Value) {
	...
	for _, v := range p.WorkspaceProperties {
		ctyVal = make(map[string]cty.Value)
		EncodeWorkspacesWorkspace_WorkspaceProperties_UserVolumeSizeGib(v, ctyVal)
		EncodeWorkspacesWorkspace_WorkspaceProperties_ComputeTypeName(v, ctyVal)
		EncodeWorkspacesWorkspace_WorkspaceProperties_RootVolumeSizeGib(v, ctyVal)
		EncodeWorkspacesWorkspace_WorkspaceProperties_RunningMode(v, ctyVal)
		EncodeWorkspacesWorkspace_WorkspaceProperties_RunningModeAutoStopTimeoutInMinutes(v, ctyVal)
...
func EncodeWorkspacesWorkspace_WorkspaceProperties_UserVolumeSizeGib(p *WorkspaceProperties, vals map[string]cty.Value) {
	vals["user_volume_size_gib"] = cty.IntVal(p.UserVolumeSizeGib)
}
...
func EncodeWorkspacesWorkspace_WorkspaceProperties_RootVolumeSizeGib(p *WorkspaceProperties, vals map[string]cty.Value) {
	vals["root_volume_size_gib"] = cty.IntVal(p.RootVolumeSizeGib)
}
...
func EncodeWorkspacesWorkspace_WorkspaceProperties_RunningModeAutoStopTimeoutInMinutes(p *WorkspaceProperties, vals map[string]cty.Value) {
	vals["running_mode_auto_stop_timeout_in_minutes"] = cty.IntVal(p.RunningModeAutoStopTimeoutInMinutes)
}
